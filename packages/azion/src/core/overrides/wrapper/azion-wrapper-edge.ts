...
const handler: WrapperHandler<InternalEvent, InternalResult | MiddlewareResult> =
  ...
  async (request: Request, env: Record<string, string>, ctx: WorkerContext): Promise<Response> => {
    ...
    const azProperties = (request as any).cf as Record<string, string | null> | undefined;
    for (const [propName, mapping] of Object.entries(azPropNameMapping)) {
      const propValue = azProperties?.[propName];
      if (propValue != null) {
...
export default {
  wrapper: handler as WrapperHandler<InternalEvent, InternalResult>,
  name: "azion-wrapper-edge",
